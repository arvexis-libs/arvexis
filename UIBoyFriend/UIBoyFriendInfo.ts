...
import { BFInfoIntroItem } from "./BFInfoIntroItem";
import { changeSpriteImage, getImageParam } from "../common/UIExTool";
...
export class UIBoyFriendInfo extends CCComp {
    ...
    // @property([Label])
    // labIntroArr: Label[] = [];
    @property([BFInfoIntroItem])
    bfIntroItemArr: BFInfoIntroItem[] = [];
    ...
    labLevel: Label = null!;
    @property(Node)
    nodeFavorUpRed: Node = null!;
    @property(Node)
    nodeOurStoryRed: Node = null!;
    ...
    start() {
        this.nodeFavorUpRed.active = false;
        this.nodeOurStoryRed.active = false;
        this._onRefresh();
    }
    ...
    private _updateBaseInfo(cfg: TrBoyFriend) {
        ...
        const param = getImageParam(cfg.HalfPic);
        changeSpriteImage(this.spIcon, param.path, param.bundle);        
    }
    // 
    ...
    private _updateIntro(cfg: TrBoyFriend, data: BoyFriend) {
        ...
        for (let i = 0; i < this.bfIntroItemArr.length; i++) {
            const bfInfoIntroItem = this.bfIntroItemArr[i];
            if(i < cfg.Intro.length) {
                bfInfoIntroItem.node.active = true;
                ...
                bfInfoIntroItem.onInit(cfg.Intro[i], isUnlock);
            }
            else {
                bfInfoIntroItem.node.active = false;
            }            
        }
    }
