...
export class HeroineDataManager
{
    ...
    //todo\lbb: 
    public GetAllIdentityProp(): number[]
    {
        let arrProp = [0, 0, 0, 0, 0]; //
        let dicIdentity = GameData.PlayerData.HeroineData.DicIdentity;
        dicIdentity.forEach((value, key) => {
            let identityId = key;
            let level = value;
            let arr = this.GetArrProp(identityId, level);
    ...
    public IsSublimingPropEnable(identityId: number)
    {
        let trIdentity = ConfigManager.tables.TbIdentity.get(identityId);
        let level = GameData.PlayerData.HeroineData.DicIdentity.get(identityId)!;
        return level >= trIdentity!.SublimingLevel;
    }
    ...
    GetArrProp(identityId: number, level:number)
    {
        ...
        result.push(TrIdentityLevel?.Prop1! || 0);
        result.push(TrIdentityLevel?.Prop2! || 0);
        result.push(TrIdentityLevel?.Prop3! || 0);
        result.push(TrIdentityLevel?.Prop4! || 0);
        result.push(TrIdentityLevel?.Prop5! || 0);
        return result;
    }
//#region 
    public getLvCur()
