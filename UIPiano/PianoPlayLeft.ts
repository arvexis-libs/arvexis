...
export class PianoPlayLeft extends PianoPlayBase {
    ...
    updateGame(): void {
        ...
        if (this.indexMusicNote >= musicLength) {
            if (!PianoPlayBase._isLeftOver) {
                PianoPlayBase._isLeftOver = true;
                this.JudgGameOver();
            }
        }
        super.updateGame();
