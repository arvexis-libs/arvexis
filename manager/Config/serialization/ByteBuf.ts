import { TextCodec } from "db://assets/script/modules/base/TextCodec";
...
export default class ByteBuf {
    ...
    WriteString(x: string) {
        const n = TextCodec.encode(x).length;
        this.WriteSize(n);
        if (n > 0) {
            this.EnsureWrite(n);
            TextCodec.encodeInto(x, this._bytes.subarray(this._writerIndex, this._writerIndex + n));
            this._writerIndex += n;
        }
    }
    ...
    ReadString(): string {
        ...
        if (n > 0) {
            this.EnsureRead(n);
            const s = TextCodec.decode(this._bytes.subarray(this._readerIndex, this._readerIndex + n));
            this._readerIndex += n;
            return s;
        }
        else {
